	...
	implements  Serializable
{
	...
	{
		vFluents.put( "INMANAGING", generatedbyassl.as.autonomicnetwork.asself_management.self_configuring.INMANAGING.getInstance() );
	}
	...
	{
		//************************ MAPPING ***********************
		if ( generatedbyassl.as.autonomicnetwork.asself_management.self_configuring.INMANAGING.getInstance().isFluentInitiated() ) 
		{
			...
			Runnable actionsRunner = new Runnable()
			{
				...
				public void run (  )
				{
					try
					{
						generatedbyassl.as.autonomicnetwork.actions.MANAGE.getInstance().perform(  );
					}
					catch ( Exception ex ) 
					{
			...
			if ( !isMappingRunning( "INMANAGING" ) ) 
			{
				Thread runActions = new Thread(thrdgrMappings, actionsRunner, "INMANAGING");
				runActions.start();
			}
		}
	}
