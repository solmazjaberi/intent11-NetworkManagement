	...
	implements  Serializable
{
	...
	{
		vFluents.put( "INREPAIRING", generatedbyassl.as.aes.controller.aeself_management.self_healing.INREPAIRING.getInstance() );
	}
	...
	{
		//************************ MAPPING ***********************
		if ( generatedbyassl.as.aes.controller.aeself_management.self_healing.INREPAIRING.getInstance().isFluentInitiated() ) 
		{
			...
			Runnable actionsRunner = new Runnable()
			{
				...
				public void run (  )
				{
					try
					{
						generatedbyassl.as.aes.controller.actions.REPAIR.getInstance().perform(  );
					}
					catch ( Exception ex ) 
					{
			...
			if ( !isMappingRunning( "INREPAIRING" ) ) 
			{
				Thread runActions = new Thread(thrdgrMappings, actionsRunner, "INREPAIRING");
				runActions.start();
			}
		}
	}
